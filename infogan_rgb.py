'''Trains infoGAN on the apple data set (adapted from the one that 
trains on the MNIST) using Keras
...
def build_and_train_models(img_array, etiquetas, latent_size=100, train_steps_in=500):
    """Load the dataset, build InfoGAN discriminator,
    generator, and adversarial models.
    Call the InfoGAN train routine.
    """
    # load APPLE dataset
    #display_one(img_array[0])
    x_train = img_array
    y_train = etiquetas
    ...
    model_name = "infogan_apples" + "_img_size_" +  str(image_size)
    # network parameters
    batch_size = 64
    train_steps = train_steps_in
    lr = 2e-4
    decay = 6e-8
...
def test_generator(generator, params, latent_size=100, num_imgs=16, dpi=200):
    label, num_labels, code1, code2, p1, p2 = params
    noise_input = np.random.uniform(-1.0, 1.0, size=[num_imgs, latent_size])
    step = 0
    if label is None:
        num_labels = num_labels
        noise_label = np.eye(num_labels)[np.random.choice(num_labels, num_imgs)]
    else:
        noise_label = np.zeros((num_imgs, num_labels))
        noise_label[:,label] = 1
        step = label

    code_std = 2
    if code1 is None:
        noise_code1 = np.random.normal(scale=0.5, size=[num_imgs, 1])
    else:
        if p1:
            a = np.linspace(-code_std, code_std, num_imgs)
            a = np.reshape(a, [num_imgs, 1])
            noise_code1 = np.ones((num_imgs, 1)) * a
        else:
            noise_code1 = np.ones((num_imgs, 1)) * code1

    if code2 is None:
        noise_code2 = np.random.normal(scale=0.5, size=[num_imgs, 1])
    else:
        if p2:
            a = np.linspace(-code_std, code_std, num_imgs)
            a = np.reshape(a, [num_imgs, 1])
            noise_code2 = np.ones((num_imgs, 1)) * a
        else:
            noise_code2 = np.ones((num_imgs, 1)) * code2
